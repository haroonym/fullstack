...
async function getCarsRoutes() {
  const { rows } = await db.query('SELECT * FROM cars JOIN owner ON cars.owner = owner.id');
  return {
    code: 200,
    data: rows,
  };
...
async function getCarsByIDRoutes(id) {
  const { rows } = await db.query('SELECT * FROM cars JOIN owner o on o.id = cars.owner where cars.id = $1', [id]);
  return {
    code: 200,
    data: rows,
  };
